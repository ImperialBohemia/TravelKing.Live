...
class TravelpayoutsConnector(BaseConnector):
    ...
    def __init__(self, vault):
        ...
        # Support both 'api_token' (Peak Fire) and 'token' (PR2) keys
        self.api_token = self.config.get("api_token") or self.config.get("token")
    ...
    def api_call(self, url_or_endpoint, method="GET", data=None, params=None):
        """Standardized Travelpayouts API caller with permanent token handling."""
        if url_or_endpoint.startswith("http"):
            url = url_or_endpoint
        else:
            url = f"{self.base_url}/{url_or_endpoint.lstrip('/')}"
        ...
        p = params or {}
        if self.marker and "marker" not in p:
